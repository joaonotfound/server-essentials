...
command.register('wemix', 'fill with mixed blocks', CommandPermissionLevel.Operator).overload(async (params, origin, output) => {
    ...
    bedrockServer.executeCommand(`fill ${we_pos1.x} ${we_pos1.y} ${we_pos1.z} ${we_pos2.x} ${we_pos2.y} ${we_pos2.z} ${params.block.getName()} ${params.id}`)
    console.log(sizeVec3, Math.abs(sizeVec1))

    for(let block = 0; block < Math.abs(sizeVec1); block++){
        const command = `setblock ${we_pos1.x - randint(sizeVec3.x)} ${we_pos1.y - randint(sizeVec3.y)} ${we_pos1.z - randint(sizeVec3.z)} ${params.block2.getName()} ${params.id2}`
