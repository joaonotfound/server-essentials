...
import { categoriesCollection } from "../../server-utils/utils/category-collection";
...
const listShopItems = async (player: Player, items: StoreItem[]) => {
    ...
    const response = await Form.sendTo(ni!, {
        ...
        buttons: [
            ...(items.map(item => ({
                text: `${beautifyItemName(item)}: $${item.price}`,
                image: { type: "path", data: item.path_image ?? `textures/items/${item.itemName.split(":")[1]}` },
            })) as FormItemButton[]),
        ],
    });
    ...
    const selectedCategory = await Form.sendTo(ni, {
        ...
        buttons: [
            ...(Array.from(categories.values()).map(category => ({
                text: category.name,
                image: category.path_image ? { type: "path", data: category.path_image } : undefined,
            })) as FormItemButton[]),
        ],
    });
